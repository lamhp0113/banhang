    ...
    if (isset($_POST['sua'])) {
      ...
      $hinh = $_FILES['hinh'];
        ...
        if ($name) {
         ...
         `Ten_hang_hoa`='$ten',`ma_code`='$ma_code',`gia`='$gia',`so_luong_hang`='$soluong',`hinh`='$img_url2',`id_loai`=$idloaisanpham WHERE id=$id";
       }else{
         $sql = "UPDATE `hang_hoas` SET 
         `Ten_hang_hoa`='$ten',`ma_code`='$ma_code',`gia`='$gia',`so_luong_hang`='$soluong',`id_loai`=$idloaisanpham WHERE id=$id";
       }
              ...
              <form action="sua.php?id=<?php echo $in1["id"]; ?>  " method="POST" enctype="multipart/form-data">
                <tbody>
                  ...
                  <tr>
                    ...
                    <label>Mã code</label>
                    <input type="text" class="form-control"  style="width:30%" value="<?php  echo $in1["ma_code"]; ?>" name="code">
